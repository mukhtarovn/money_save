...
def last_month(request):
    ...
    if exp_today['sum__sum'] == None:
        exp_today['sum__sum'] = 0
    inc_yesterday = Income.objects.filter(user=request.user, time_create__month=last).aggregate(Sum("sum"))
    if inc_yesterday['sum__sum'] == None:
        inc_yesterday['sum__sum'] = 0
    exp_yesterday = NecessaryExpenses.objects.filter(user=request.user, time_create__month=last).aggregate(
        Sum("sum"))
    if exp_yesterday['sum__sum'] == None:
        exp_yesterday['sum__sum'] = 0
    inc_two_day = Income.objects.filter(user=request.user, time_create__month=two).aggregate(Sum("sum"))
    if inc_two_day['sum__sum'] == None:
        inc_two_day['sum__sum'] = 0
    exp_two_day = NecessaryExpenses.objects.filter(user=request.user, time_create__month=two).aggregate(Sum("sum"))
    if exp_two_day['sum__sum'] == None:
        exp_two_day['sum__sum'] = 0
    inc_three_day = Income.objects.filter(user=request.user, time_create__month=three).aggregate(Sum("sum"))
    if inc_three_day['sum__sum'] == None:
        inc_three_day['sum__sum'] = 0
    exp_three_day = NecessaryExpenses.objects.filter(user=request.user, time_create__month=three).aggregate(Sum("sum"))
    if exp_three_day['sum__sum'] == None:
        exp_three_day['sum__sum'] = 0
    inc_four_day = Income.objects.filter(user=request.user, time_create__month=four).aggregate(Sum("sum"))
    if inc_four_day['sum__sum'] == None:
        inc_four_day['sum__sum'] = 0
    exp_four_day = NecessaryExpenses.objects.filter(user=request.user, time_create__month=four).aggregate(Sum("sum"))
    if exp_four_day['sum__sum'] == None:
        exp_four_day['sum__sum'] = 0
    inc_five_day = Income.objects.filter(user=request.user, time_create__month=five).aggregate(Sum("sum"))
    if inc_five_day['sum__sum'] == None:
        inc_five_day['sum__sum'] = 0
    exp_five_day = NecessaryExpenses.objects.filter(user=request.user, time_create__month=five).aggregate(Sum("sum"))
    if exp_five_day['sum__sum'] == None:
        exp_five_day['sum__sum'] = 0
    inc_six_day = Income.objects.filter(user=request.user, time_create__month=six).aggregate(Sum("sum"))
    if inc_six_day['sum__sum'] == None:
        inc_six_day['sum__sum'] = 0
    exp_six_day = NecessaryExpenses.objects.filter(user=request.user, time_create__month=six).aggregate(Sum("sum"))
    if exp_six_day['sum__sum'] == None:
        exp_six_day['sum__sum'] = 0
